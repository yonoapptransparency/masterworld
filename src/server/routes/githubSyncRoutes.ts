...
githubSyncRouter.post("/api/github-sync/commit-atomic", verifyAdminToken, async (req, res) => {
  try {
    const { owner, repo, branch = 'main', files, message, token } = req.body || {};
    let activeToken = token || process.env.PAT;
    if (!owner || !repo || !activeToken || !files || !Array.isArray(files)) {
    ...
    const refRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/git/ref/heads/${branch}`, {
      headers: { 'Authorization': authHeader, 'User-Agent': 'node-fetch' }
    });
    if (!refRes.ok) {
        const err = await refRes.json().catch(()=>({}));
        return res.status(refRes.status).json({ message: `Failed to get ref: ${err.message}` });
    }
    ...
    const treeRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/git/trees`, {
      method: 'POST',
      headers: { 'Authorization': authHeader, 'User-Agent': 'node-fetch', 'Content-Type': 'application/json' },
      body: JSON.stringify({ base_tree: latestCommitSha, tree: treeItems })
    });
    if (!treeRes.ok) {
        const err = await treeRes.json().catch(()=>({}));
        return res.status(treeRes.status).json({ message: `Failed to create tree: ${err.message}` });
    }
    ...
    if (!commitRes.ok) {
        ...
        return res.status(commitRes.status).json({ message: `Failed to create commit: ${err.message}` });
    }
    ...
    if (!updateRes.ok) {
        ...
        return res.status(updateRes.status).json({ message: `Failed to update ref: ${err.message}` });
    }

    return res.json({ success: true, commitSha: commitData.sha, targetRepo: repo });
  } catch (err: any) {
    console.error("Server GitHub atomic commit error:", err);
