...
export async function commitFileToGitHub({
  ...
  if (!response.ok) {
    ...
    if (contentType && contentType.includes('text/html')) {
      // If we got HTML back, it's likely a 404 or 500 page from the host (e.g. Vercel)
      throw new Error(`Server returned HTML instead of JSON (${response.status}). This usually indicates a routing issue or a backend crash. Check if the /api routes are correctly deployed.`);
    }

    try {
      const errJSON = JSON.parse(errText);
      // Handle both 'message' and 'error' keys from server
      errMsg = errJSON.message || errJSON.error || errText;
    } catch (_) {
      // If not JSON, use the raw text or status code if text is empty
      if (!errMsg) errMsg = `HTTP Error ${response.status}`;
    }
