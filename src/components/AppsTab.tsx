...
// Sub-component for Screenshots Editing
function ScreenshotsEditor({ initialScreenshots }: { initialScreenshots: string[] }) {
...
const AppsTab = React.memo(({ appsList, editingAppId, setEditingAppId, handleDeleteApp, handleSaveApp, categories, saving, lastSavedAt }: any) => {
          ...
          const content = (
            ...
            <form onSubmit={handleSaveApp} className="flex flex-col h-full overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 pb-40 sm:pb-48 space-y-6 custom-scrollbar bg-slate-50/30 dark:bg-slate-900/10">
                ...
                <div className={activeFormTab === 'screenshots' ? 'animate-fade-in space-y-4' : 'hidden'}>
              ...
              <div className="p-4 sm:p-5 border-t border-slate-100 dark:border-slate-800/80 flex justify-between items-center gap-3 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md shrink-0 sticky bottom-0 z-20 pb-safe shadow-lg">
                <div className="flex items-center gap-2">
                  {saving && (
                    <span className="text-xs font-semibold text-blue-600 dark:text-blue-400 animate-pulse flex items-center gap-1.5">
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" /> Storing to Cloud Database...
                    </span>
                  )}
                  {!saving && lastSavedAt && (
                    <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                      <Save className="w-3.5 h-3.5 text-emerald-500" /> Saved at {lastSavedAt}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-3">
                  ...
                  >
                    {saving ? <><RefreshCw className="w-4 h-4 animate-spin"/> Synchronizing...</> : <><Save className="w-4 h-4"/> Publish App Configuration</>}
                  </button>
                </div>
              </div>
