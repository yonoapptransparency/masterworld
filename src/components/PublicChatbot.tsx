/**
 * PublicChatbot directory tour helper chatbot
 * Connects directly to server-side Gemini intelligence for assisting with game walkthrough guidelines.
 */
...
export default function PublicChatbot() {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'assistant'; content: string }[]>([
    { role: 'assistant', content: 'Hello! I am the Rummydex Assistant. How can I help you navigate the directory today?' }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        const updateState = (text: string, force = false) => {
          const now = Date.now();
          if (force || now - lastUpdateTime > 45) {
            lastUpdateTime = now;
            if (updateTimeout) {
        ...
        try {
          ...
          // Force flush any pending text at the end
          updateState(accumulatedText, true);
        } finally {
          if (updateTimeout) clearTimeout(updateTimeout);
          reader.releaseLock();
        }
              ...
              {isLoading && (
                ...
                >
                  ...
                  <div className="px-5 py-4 bg-white/60 dark:bg-neutral-800/60 backdrop-blur-md shadow-sm border border-white/50 dark:border-white/10 rounded-2xl rounded-bl-sm flex items-center gap-1.5 h-[46px]">
                    <div className="w-1.5 h-1.5 bg-neutral-400 rounded-full animate-pulse" style={{ animationDelay: '0ms', animationDuration: '1s' }} />
                    <div className="w-1.5 h-1.5 bg-neutral-400 rounded-full animate-pulse" style={{ animationDelay: '150ms', animationDuration: '1s' }} />
                    <div className="w-1.5 h-1.5 bg-neutral-400 rounded-full animate-pulse" style={{ animationDelay: '300ms', animationDuration: '1s' }} />
                  </div>
                </motion.div>
              )}
