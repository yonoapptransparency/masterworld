...
export default function ClearanceButton({ appId, status, variant = 'default' }: ClearanceButtonProps) {
  ...
  const isRealValue = (id: string | undefined): boolean => {
    ...
    // Cloudflare Turnstile site keys are alphanumeric string tokens (e.g., 0x4AAAAAA...)
    // If it contains password special characters (#, @, !, $, ^, &, *), it is a mock string and should be ignored.
  ...
  const triggerHandshake = async (targetWin: Window | null = null) => {
    ...
    try {
      ...
      if (!challengeResponse.ok) {
        ...
        if (contentType.includes('application/json')) {
          ...
          if (errMsg && typeof errMsg === 'object') {
            errMsg = (errMsg as any).message || (errMsg as any).error || JSON.stringify(errMsg);
          }
        }
        throw new Error(errMsg || 'Initialization request was denied by security checkpoint.');
      }
      ...
      if (!tokenContentType.includes('application/json')) {
        throw new Error('Verification process returned an invalid response. Please try again.');
      }
      ...
      if (safeErrorStr.includes('Unexpected token') || safeErrorStr.includes('not valid JSON') || safeErrorStr.includes('JSON.parse')) {
        safeErrorStr = 'Verification service is temporarily offline. Please refresh and try again.';
      } else if (safeErrorStr.includes('Failed to fetch') || safeErrorStr.includes('NetworkError') || safeErrorStr.includes('load resource')) {
        safeErrorStr = 'Connection lost. Please check your internet connection and try again.';
      }
      setErrorMsg(safeErrorStr || 'Initialization did not complete. Please retry.');
      setPhase('error');
      setTimeout(resetState, 4500);
    }
  };
            ...
            {phase === 'ready' ? (
              ...
              <motion.button 
                ...
                className={variant === 'compact' 
                  ? `w-full premium-action-btn premium-action-btn-blowing text-white !text-white font-bold py-2.5 px-5 rounded-xl flex items-center justify-center gap-1.5 transition-colors text-sm shadow-md h-[44px] ${isGenerating ? 'opacity-70 cursor-not-allowed' : ''}`
                  : `w-full sm:w-96 premium-action-btn premium-action-btn-blowing text-white !text-white font-bold py-4 px-10 rounded-2xl flex items-center justify-center gap-2 transition-colors shadow-sm text-base shrink-0 ${isGenerating ? 'opacity-70 cursor-not-allowed' : ''}`
                }
              >
                {isGenerating ? (
                  <motion.div 
                    initial={{ opacity: 0 }} 
                    animate={{ opacity: 1 }} 
                    className="flex items-center gap-2 text-white !text-white"
                  >
                    <Loader2 className={`animate-spin text-white !text-white ${variant === 'compact' ? 'w-4 h-4' : 'w-5 h-5'}`} /> 
                    <span className="font-bold text-white !text-white">{variant === 'compact' ? 'Verifying...' : 'Loading...'}</span>
                  </motion.div>
                ) : (
                  <motion.span 
                    initial={{ opacity: 0 }} 
                    animate={{ opacity: 1 }}
                    className={variant === 'compact' ? "flex items-center gap-1.5 font-bold text-white !text-white" : "text-white !text-white font-bold"}
                  >
