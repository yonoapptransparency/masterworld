          ...
          banners.map((banner: any, index: number) => (
            <div key={banner.id || index} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-5">
              <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-800">
                ...
                <button aria-label={`Remove banner ${index + 1}`} onClick={() => onRemove(index, banner.id)} className="text-rose-500 hover:text-rose-600 p-1 cursor-pointer border-0 bg-transparent"><Trash2 className="w-4 h-4" /></button>
              </div>
