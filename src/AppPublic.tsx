...
function AppContent() {
  ...
  useEffect(() => {
    ...
    if (path.startsWith('/app/')) {
      ...
      const prefix = path.startsWith('/info/') ? '/info/' : (path.startsWith('/gateway/') ? '/gateway/' : '/moredetail/');
      const slug = decodeURIComponent(path.split(prefix)[1]?.split('/')[0]?.split('?')[0] || '');
      const app = apps.find((a: any) => a?.slug?.toLowerCase() === slug.toLowerCase());
      if (app && app.icon_url) {
        targetUrl = app.icon_url;
      }
    } else {
      // Check fallback root-level routes
