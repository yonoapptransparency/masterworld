...
// Use a simple split/join to replace exactly the block
const startMarker = 'communityRouter.get("/api/v1/public/community/reviews/:appId"';
const endMarker = 'communityRouter.get("/api/v1/public/community/stats/:appId"';

if (code.indexOf(startMarker) > -1 && code.indexOf(endMarker) > -1) {
  const before = code.substring(0, code.indexOf(startMarker));
  const after = code.substring(code.indexOf(endMarker));
  
  const middle = `communityRouter.get("/api/v1/public/community/reviews/:appId", async (req: any, res: any) => {
...
`;
  ...
  fs.writeFileSync('src/server/routes/communityRoutes.ts', before + middle + after);
